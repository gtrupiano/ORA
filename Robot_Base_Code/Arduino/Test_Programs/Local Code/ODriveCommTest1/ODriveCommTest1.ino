...
void sendEncoderSpeed(float speed)
{
    CanMsg msg;
    msg.id = 5; // CAN message ID for the encoder speed
    msg.len = 8; // Length of the CAN message
    memcpy(msg.buffer, &speed, sizeof(speed)); // Copy speed value to the CAN message buffer

    CAN.beginPacket(msg.id);
    CAN.write(msg.buffer, msg.len);
    CAN.endPacket();
}
...
void ODriveMovement(int8_t angularVelIn, int8_t linearVelIn)
{
  ...
  leftMotorTPS = (leftMotorTPS / (2 * M_PI)) * 51;
  rightMotorTPS = (rightMotorTPS / (2 * M_PI)) * -51;
...
void autonEncoderData()
{
  ...
  if (odrv0_user_data.received_feedback) 
  {
    ...
    sendEncoderSpeed(odrv0EncoderVel);
  }
  ...
  if (odrv1_user_data.received_feedback) 
  {
    ...
    //sendEncoderSpeed(odrv1EncoderVel);
  }
