...
#define EStop 4 // Hardware EStop
#define IMU_SDA 2 // On AtMega32U4 Connected to Pin 19 (PD1)
#define IMU_SCL 3 // On AtMega32U4 Connected to Pin 18 (PD0) // CHECK

// LED Declarations
#define LEDPWR_R 5 // On AtMega32U4 Connected to Pin 25 (PD4)
#define LEDPWR_G 6 // On AtMega32U4 Connected to Pin 26 (PD6)
#define LEDPWR_B 7 // On AtMega32U4 Connected to Pin 27 (PD7)
...
bool previousEStopButton = false;
bool previousAutonButton = false;
...
void loop() 
{
  ...
  if(Wire.available() >= sizeof(int) + sizeof(int) + sizeof(bool)) // could combine with If statement below but this is easier to read for now
  {
    ...
    Serial.print("Received from ESP32 - Autonomous State Button: ");
...
*/
  ...
  if (previousEStopButton != EStopButton) // Did state change
  {
    // EStopButton went from low to high
    if (EStopButton) 
    {
      // If EStop is currently inactive, activate it
      ODriveEStop();
      EStopState = true;
    }
    else
  ...
  // Allows Autonomous State to be toggled
  if (previousAutonButton != AutonButton) // Did state change
  {
    // AutonButton went from low to high
    if (AutonButton) 
    {
      AutonState = true;
    }
  ...
  previousEStopButton = EStopButton;
  previousAutonButton = AutonButton;
  ...
  if(EStopState == true)
  {
    digitalWrite(LEDPWR_R, HIGH);
    digitalWrite(LEDPWR_G, LOW);
    digitalWrite(LEDPWR_B, LOW);
  }
  else
  {
    if (AutonState ==  true)
    {
      digitalWrite(LEDPWR_R, LOW);
      digitalWrite(LEDPWR_G, LOW);
      digitalWrite(LEDPWR_B, HIGH);
    }

    else
    {
      digitalWrite(LEDPWR_R, LOW);
      digitalWrite(LEDPWR_G, HIGH);
      digitalWrite(LEDPWR_B, LOW);
    }
  }
...
void ODriveMovement(double verticalVelocity, double horizontalVelocity) // redo math for turns/second
{
  ...
  const double maxVelocity = 74; // IGVC rules, should be meters/second units but not sure
