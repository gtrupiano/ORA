...
void loop() 
{
  ...
  else
  {
    ...
    ODriveMovement(verticalMov);
  }
}

// Function to send commands to ODrive via CAN bus
void ODriveMovement(double verticalVelocity,double horizontalVelocity) // Need to make them percentages in order to get robot turning
{
  // Gives inital torque and structures message based on CANSimple
  float feedfowardTorque = 0.1;
  byte velData[] = {verticalVelocity, feedfowardTorque};
