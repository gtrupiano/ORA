...
bool controllerCompletion = false;
...
{
  // Checks to see if data received from PS4 controller. If not sets values from controller to predefined variables
  if(controllerCompletion)) //some flag that shows that it accomplished a new controller reading)*/
  {
    // Read controller values
    if (abs(PS4.RStickY()) > deadband) // Implements deadband
    {
      linearMov = PS4.RStickY(); // Reads Y position for linear movement
    }
    else
    {
      linearMov = 0;
    ...
    {
      angularMov = PS4.LStickX(); // Reads X position for angular movement
    }
    else 
    {
      angularMov = 0;
    ...
    EStopState = PS4.R1(); // Reads right bumper state (EStop State)
    AutonState = PS4.L1(); // Reads left bumper state (Autonomous State)
